...
ConcGuanajuato$Nhog <- 1

#DECILES 

#Attaching the data frame
attach(ConcGuanajuato) #this is for not writing the name of the data frame and the $ ever time

#Sort Conc according to INGCOR, FOLIOVIV, FOLIOHOG
ConcGuanajuato<- orderBy (~+ing_cor+folioviv+foliohog, data=ConcGuanajuato) #this give us the households sorted by total income

#Adding the values of the expansion FACTOR. The sum is 34 million, which is the number of households in the country.
tot_hogares<-sum(ConcGuanajuato$factor,to.data.frame=TRUE)
...
ConcGuanajuato$tam_dec<-tam_dec

# Creating Deciles of Income 

ConcGuanajuato$MAXT<-ConcGuanajuato$ing_cor #vamos a crear en esta base la variable MAXT que es una copia de la columna de ingresos

ConcGuanajuato<-ConcGuanajuato[with(ConcGuanajuato, order(rank(MAXT))),]  #lo que hicimos aqu? fue reordenar la base con respecto a MAXT. Cosa que en realidad, ya estaba.

ConcGuanajuato$ACUMULA<-cumsum(ConcGuanajuato$factor) #aqu? creamos una variable de suma acumulada del FACTOR de viviendas.
...
for(i in 1:9)
{
  a1<-ConcGuanajuato[dim(ConcGuanajuato[ConcGuanajuato$ACUMULA<tam_dec*i,])[1]+1,]$factor
  ConcGuanajuato<-rbind(ConcGuanajuato[1:(dim(ConcGuanajuato[ConcGuanajuato$ACUMULA<tam_dec*i,])[1]+1),],
                             ConcGuanajuato[(dim(ConcGuanajuato[ConcGuanajuato$ACUMULA<tam_dec*i,])[1]+1):dim(ConcGuanajuato[1])[1],])
  
  b1<-tam_dec*i-ConcGuanajuato[dim(ConcGuanajuato[ConcGuanajuato$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  ConcGuanajuato[(dim(ConcGuanajuato[ConcGuanajuato$ACUMULA<tam_dec*i,])[1]+1),]$factor<-b1
  ConcGuanajuato[(dim(ConcGuanajuato[ConcGuanajuato$ACUMULA<tam_dec*i,])[1]+2),]$factor<-(a1-b1)
}

#aqu? estamos creando otra variable de suma acumulada del n?mero de hogares
ConcGuanajuato$ACUMULA2<-cumsum(ConcGuanajuato$factor)

#aqu? estamos creando una variable que se llama decil que solo tiene ceros
ConcGuanajuato$DECIL<-0
...
ConcGuanajuato[(ConcGuanajuato$ACUMULA2<=tam_dec),]$DECIL<-1
...
for(i in 1:9)
{
  ConcGuanajuato[((ConcGuanajuato$ACUMULA2>tam_dec*i)&(ConcGuanajuato$ACUMULA2<=tam_dec*(i+1))),]$DECIL<-(i+1)
}

# a lo que le qued? cero (que es la ?ltima observaci?n), ponle el decil 10
ConcGuanajuato[ConcGuanajuato$DECIL%in%"0",]$DECIL<-10

setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Guanajuato")
write.dbf(ConcGuanajuato,file="ConcGuanajuato2010.dbf")
...
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Guanajuato")
write.dbf(ConcGuanajuato,file="ConcGuanajuato2018.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Guanajuato")
Conc2010<-read.dbf("ConcGuanajuato2010.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Guanajuato por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Guanajuato por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Guanajuato por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Guanajuato por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Guanajuato por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Guanajuato INDIGENA por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Guanajuato INDIGENA por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Guanajuato INDIGENA por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Guanajuato INDIGENA por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Guanajuato INDIGENA por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Guanajuato NO por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Guanajuato NO por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Guanajuato NO por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Guanajuato NO por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Guanajuato NO por fuente por DECIL LS 2010.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Guanajuato")
Conc2018<-read.dbf("ConcGuanajuato2018.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Guanajuato Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Guanajuato Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Guanajuato Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Guanajuato Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Guanajuato Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Guanajuato INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Guanajuato INDIGENA Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Guanajuato INDIGENA Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Guanajuato INDIGENA Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Guanajuato INDIGENA Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Guanajuato NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Guanajuato NO Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Guanajuato NO Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Guanajuato NO Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Guanajuato NO Ingresos por fuente por DECIL LS 2018.dbf")
...
Deciles2010<-read.dbf("Guanajuato por fuente por DECIL estimaciones 2010.dbf")
names(Deciles2010)[1]<-c("ingcor2010")

Deciles2018<-read.dbf("Guanajuato Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010No<-read.dbf("Guanajuato NO por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010No)[1]<-c("No2010")

deciles2018No<-read.dbf("Guanajuato NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010indigena<-read.dbf("Guanajuato INDIGENA por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010indigena)[1]<-c("Indigena2010")

deciles2018indigena<-read.dbf("Guanajuato INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
